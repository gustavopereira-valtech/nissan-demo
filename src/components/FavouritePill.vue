<template>
  <span>
    <div
      ...
      :key="index"
      class="flex py-2 text-sm px-4 back-gray rounded-2xl justify-between"
    >
      <img
      ...
      <div class="mx-4 self-center flex flex-col">
        <p>{{ topic.title }}</p>
        <p class="italic text-accent">Welcoming</p>
      </div>
...
export default {
  ...
  computed: {
    topics() {
      return this.store.data.trainings[0].topics.filter((training) =>
        this.store.favs.includes(training.id)
      )
    },
  },
  methods: {
